...
import { ChevronLeft, ChevronRight, ChevronUp, ChevronDown, Loader2, Expand, Download } from "lucide-react";
...
export const PDFViewerModal = ({ open, onOpenChange, attachments, startIndex = 0 }: Props) => {
    ...
    try {
      if (viewerRef.current && !autoFitDoneRef.current) {
        ...
        if (viewport?.height && available > 0) {
          ...
          const nextScale = Math.max(0.5, autoFitScale + 0.1); // Start one zoom level in
          setScale(nextScale);
  ...
  const downloadPDF = () => {
    if (!current || !blobUrl) return;
    
    const link = document.createElement('a');
    link.href = blobUrl;
    link.download = current.attachment_title || 'document.pdf';
