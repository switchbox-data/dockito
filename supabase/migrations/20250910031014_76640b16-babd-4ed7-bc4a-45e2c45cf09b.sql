-- Fix the ORDER BY clause in get_dockets_with_filing_counts function
...
BEGIN
  ...
  ORDER BY 
    CASE 
      WHEN p_sort_by = 'filing_count' AND p_sort_order = 'desc' THEN COALESCE(fc.filing_count, 0)
    END DESC NULLS LAST,
    CASE 
      WHEN p_sort_by = 'filing_count' AND p_sort_order = 'asc' THEN COALESCE(fc.filing_count, 0)
    END ASC NULLS LAST,
    CASE 
